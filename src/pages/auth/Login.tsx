...
const getInitialFormData = (): FormData => {
  // Kiểu trả về để rõ ràng
  try {
    const storedData = localStorage.getItem("express_control_account");
    if (storedData) {
      const parsedData = JSON.parse(storedData);
      // Validate parsed data before returning it.  Important!
      if (
  ...
  } catch {} // Bắt lỗi, nhưng không chỉ trả về.
  return { phone: "", password: "", remember: false }; // Luôn trả về giá trị mặc định hợp lệ
};
...
const Login = () => {
  ...
  const { phone, password, remember } = formData;
  const { addToast, ToastContainer } = useToast();

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value, type, checked } = e.target;
    setFormData((prev: FormData) => ({
      // Khai báo kiểu cho prev state
      ...prev,
  ...
  const handleLocalStorage = () => {
    if (formData.remember) {
      localStorage.setItem("express_control_account", JSON.stringify(formData));
    } else {
      localStorage.removeItem("express_control_account");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await dispatch(
      loginUser({ phone, password, role: "onwer" })
    );
    ...
    <div className="w-full border shadow-lg bg-white dark:bg-gray-800 lg:translate-x-1/2 lg:translate-y-20 lg:w-1/2 lg:border-l-2">
      ...
      <div className="w-full p-10 sm:p-12.5 lg:p-17.5">
        ...
        <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
          Sign In to Express Control Admin
        </h2>
